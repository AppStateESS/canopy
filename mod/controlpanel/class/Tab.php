...
class PHPWS_Panel_Tab
{
    ...
    public function getLink($addTitle = true, $active = false)
    {
        if ($addTitle) {
            $title = $this->getTitle();
            ...
            if ($this->_strict) {
                return sprintf('<a class="nav-link %s" href="%s"%s>%s</a>',
                        $activeClass, $link, $link_title, $title);
            } elseif ($this->_secure) {
                $authkey = Current_User::getAuthKey();
                return sprintf('<a class="nav-link %s" href="%s&amp;tab=%s&amp;authkey=%s"%s>%s</a>',
                        $activeClass, $link, $this->id, $authkey, $link_title,
                        $title);
            } else {
                return sprintf('<a class="nav-link %s" href="%s&amp;tab=%s"%s>%s</a>',
                        $activeClass, $link, $this->id, $link_title, $title);
            }
        } else {
            return $this->link;
        }
