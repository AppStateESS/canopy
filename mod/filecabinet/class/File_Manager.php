...
class FC_File_Manager
{
    ...
    public function imageOnly($folder = true)
    {
    ...
    public function startView()
    {
        ...
        $document_img = sprintf('<img src="%smod/filecabinet/img/file_manager/file_type/document200.png" title="%s"/>',
                PHPWS_SOURCE_HTTP, 'Add a document or a document folder');
        ...
        $media_img = sprintf('<img src="%smod/filecabinet/img/file_manager/file_type/media200.png" title="%s"/>',
                PHPWS_SOURCE_HTTP, 'Add a video or sound file');
    ...
    public function folderIcons(&$tpl)
    {
        ...
        if ((!$force_type || $force_type == DOCUMENT_FOLDER) && (!$this->lock_type || in_array(FC_DOCUMENT,
                        $this->lock_type))) {
            ...
            $document_img = sprintf('<img src="%s/mod/filecabinet/img/file_manager/file_type/%s" title="%s" />',
                    PHPWS_SOURCE_HTTP, $icon_name, 'View document folders');
        ...
        if ((!$force_type || $force_type == IMAGE_FOLDER) && (!$this->lock_type || in_array(FC_IMAGE,
                        $this->lock_type))) {
            ...
            $image_img = sprintf('<img src="%s/mod/filecabinet/img/file_manager/file_type/%s" title="%s"/>',
                    PHPWS_SOURCE_HTTP, $icon_name, 'View image folders');
        ...
        if ((!$force_type || $force_type == MULTIMEDIA_FOLDER) && (!$this->lock_type || in_array(FC_MEDIA,
                        $this->lock_type))) {
            ...
            $media_img = sprintf('<img src="%s/mod/filecabinet/img/file_manager/file_type/%s" title="%s"/>',
                    PHPWS_SOURCE_HTTP, $icon_name, 'View media folders');
    ...
    public function folderContentView()
    {
        ...
        if (empty($this->current_folder) || empty($this->folder_type)) {
            javascript('alert',
                    array('content' => 'Problem with opening browser page. Closing File Manager window.'));
        ...
        switch ($this->folder_type) {
            case IMAGE_FOLDER:
                ...
                $js['confirmation'] = sprintf('This image is larger than the %s x %s limit. Do you want to resize the image to fit?',
            ...
            case MULTIMEDIA_FOLDER:
                ...
                $js['confirmation'] = sprintf('This media is larger than the %s x %s limit. Do you want to resize the media to fit?',
                        $this->max_width, $this->max_height);
